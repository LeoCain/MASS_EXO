...
class PPO(object):
	...
	def SaveModel(self):
		self.model.save('../nn_crip/current.pt')
		self.muscle_model.save('../nn_crip/current_muscle.pt')
		
		if self.max_return_epoch == self.num_evaluation:
			self.model.save('../nn_crip/max.pt')
			self.muscle_model.save('../nn_crip/max_muscle.pt')
		if self.num_evaluation%100 == 0:
			self.model.save('../nn_crip/'+str(self.num_evaluation//100)+'.pt')
			self.muscle_model.save('../nn_crip/'+str(self.num_evaluation//100)+'_muscle.pt')

	def LoadModel(self,path):
		self.model.load('../nn_crip/'+path+'.pt')
		self.muscle_model.load('../nn_crip/'+path+'_muscle.pt')
...
def Plot(y,title,num_fig=1,ylim=True):
	...
	if ylim:
		plt.ylim([0,1])
	plt.pause(0.001)
	plt.savefig('../png/Reward_Graph_crip.png')
	plt.close()
